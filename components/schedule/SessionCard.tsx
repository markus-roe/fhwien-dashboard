...
export const SessionCard = ({ session, onClick }: SessionCardProps) => {
  ...
  if (session.type === 'lecture') {
    colors = typeColors.lecture[session.locationType] || typeColors.lecture['on-campus']
  } else {
  ...
  return (
    <div
      className={`p-1.5 ${colors.bg} relative transition-colors ${onClick ? 'cursor-pointer' : ''} overflow-hidden`}
      onClick={onClick}
    >
      <div
        className={`h-full w-full bg-white border rounded-lg p-3 shadow-sm border-l-[3px] ${colors.border} flex flex-col justify-between hover:shadow-md hover:-translate-y-0.5 transition-all group overflow-hidden`}
      >
        <div className="min-w-0 flex-1 flex flex-col">
          <div className="flex justify-between items-start gap-2 mb-1">
            <p className={`text-[10px] font-bold uppercase tracking-wide ${colors.text} shrink-0`}>
            ...
            {session.type === 'coaching' && session.participants && (
              <div className="flex -space-x-1.5 shrink-0">
                {Array.from({ length: Math.min(session.participants, 2) }).map((_, i) => (
                  <div
                    key={i}
          ...
          <p className="text-sm font-semibold text-zinc-900 leading-tight line-clamp-2 break-words">{session.title}</p>
          <p className="text-[11px] text-zinc-500 mt-1 truncate">
            {session.program} • {session.locationType === 'online' ? 'Online' : session.location}
          </p>
        </div>
      </div>
    </div>
