...
export const TodayList = ({ date, sessions, onSessionClick }: TodayListProps) => {
  return (
    <div className="bg-white rounded-xl border border-zinc-200 p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-medium text-zinc-900">Heute</h3>
        <span className="text-[10px] text-zinc-400">{format(date, 'MMM d')}</span>
      </div>
      ...
      <div className="space-y-3 relative">
        ...
        {sessions.map((session, index) => (
          <div
            ...
            className={`group flex gap-3 items-start relative z-10 ${
          ...
          >
            ...
            ></div>
            <div className={`flex-1 min-w-0 ${index < sessions.length - 1 ? 'pb-3 border-b border-zinc-50' : ''}`}>
